...
public: 	
	...
	void set_ring_pxl(const geometry_msgs::PoseStamped::ConstPtr &msg)
	{
		if(getPxl and !newAvg)
		{
			...
			{
				ROS_INFO("Averaging");
				ring_pxl.pose.position.x /= 10;
				ring_pxl.pose.position.y /= 10;
				counter  = 0;
				newAvg = true;
	...
	void approach_coord(double x, double y)
	{
		...
		if(map_avaiable){
			...
			if(a.aligning){
				...
				else if(tries > 2)
				{
					...
					a.approach_coord(a.ring_pos.pose.position.x, a.ring_pos.pose.position.y);
					a.aligning = false;
					a.getPxl = false;
					a.pickUp = true;
					tries = 0;
			...
			{
				a.getPxl = true;
				if(a.init_time.header.stamp < a.ring_pxl.header.stamp)
				{
					a.aligning = true;
					a.check = false;
					a.getPxl = false;
