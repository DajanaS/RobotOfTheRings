...
int determineLying(int id, string name){
...
int main(int argc, char **argv) {
    ...
    GoalSender sender(n);
    ros::Subscriber sub = n.subscribe("speachFiltered", 1000, &GoalSender::getReply, &sender);
    ...
    for(int i=0; i<6; i++){
        if(onField[i].gender == FEMALE){
			...
			int l = sender.determineLying(i, onField[idMan].name);
            if(l == 1){
				cout<<"Lying!"<<endl;
                tellingTruth = 0;
            } else {
    ...
    if(onField[knowsWhereRing].gender == FEMALE && onField[knowsWhichRing].gender == FEMALE){
        ...
        for(int i=0; i<6; i++){
			if(onField[i].gender == MALE){
                ...
                lWhere = sender.determineLying(knowsWhereRing, onField[i].name);
				lWhich = sender.determineLying(knowsWhichRing, onField[i].name);
				break;
			}
		}
		...
		if(lWhich){
				mRing = sender.femaleWhichLying(onField[knowsWhichRing].name);
		} else {
		...
		if(lWhere){
			    cyl = sender.femaleWhereLying(onField[knowsWhereRing].name);
		} else {
        ...
        if(onField[knowsWhereRing].gender == MALE){
                sender.pxl_goal(onField[knowsWhereRing].x, onField[knowsWhereRing].y, 0);
                cyl = sender.maleWhere(onField[knowsWhereRing].name);
                int whichLL = sender.determineLying(knowsWhichRing, onField[knowsWhereRing].name);
            ...
            mRing = sender.maleWhich(onField[knowsWhichRing].name);
                ...
                int whereLL = sender.determineLying(knowsWhereRing, onField[knowsWhichRing].name);
